...
存在理由：scripts/build-core-airports.ts 曾只讀 legacy 的 flight-list.json，
         漏掉 Top-1000 戰役的新格式班表 scripts/flights/{ICAO}/{date}.json，
         導致 manifest 的 complete 低報（2026-08-22 實測 74 vs core-only 189）。
...
legacy={}
legacy_path=f'{base}/flight-list.json'
if os.path.exists(legacy_path): legacy=json.load(open(legacy_path))
for completed in legacy.get('completed',[]):
    icao=completed.split(':')[0]
    if icao: core.add(icao.upper())
...
core_full={i:fds for i,fds in full.items() if i in core}
core_d218=[i for i,fds in core_full.items() if '2026-02-18' in fds]
print('主動查詢機場數:', len(core))
print('core-only 至少一天「抓滿」:', len(core_full))
print('core-only 2026-02-18 抓滿:', len(core_d218))
# 100% 且 >=50 班
