...
> **狀態快照 @ 2026-08-21（本期 666,000 credits 已用盡）**：戰役 **34,449 / 65,421 = 52.7%（過半）**，還剩 30,972 條 ≈ 1.24M credits。8/20–21 三段共抓 **16,013 筆**、花 **~658,480 credits**、**零最終失敗**（16 筆 5xx／網路失敗全數 retry 救回）。額度剩 ~7,720（193 條）。
...
| — | **合計** | **16,446** | **16,013** | **444** | **0（最終）** | **~658,480** | **14.6h** |

- **🏆 Batch 1 100% 完成**：全球運量前 39 大 megahub 在 2/18 的所有起降軌跡到位（`--rank 1-39 --dry-run` 複驗**待處理 0 筆**）。
- **進度**：28.6% → **52.7%**；機場檔 1,845 → **2,299 座**；不重複航班 66,078 → **82,091**；Atlas 2,250 → **2,619 點**；track-done 66,189 → **82,202**。
...
| 優先 | 範圍 | 待抓（現值）| credits |
|---|---|---|---|
| 1 | Batch 2 收尾 `--rank 40-125` | 2,888 | ~116K |
...
| **Batch 2（目前批次）** | 40–125 | 15,040 | **2,888** | 116K |
