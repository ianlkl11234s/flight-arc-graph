...
> **狀態快照 @ 2026-08-20（新計費週期開工）**：戰役 **18,706 / 65,421 = 28.6%**，剩 46,715 筆 ≈ 1.87M credits。上期（7/20–8/19）額度已用盡；**本期 dashboard 滿額 666,000，用戶核准全押戰役**。錨點已更新：`manual_remaining=666000` / `as_of=2026-08-20` / `track_done_baseline=66189`（三欄一起改，只改額度會把上期花費重複扣一次；`as_of` 是「日」粒度，同日二次重設會重複計帳）。
> **✅ 線上站台是最新的**（8/09 全鏈部署完結，1,845 座 / 66,078 筆），沒有待部署的東西。
> 💰 成本：時刻表 **3 credits/筆**、軌跡 **40 credits/筆**；估算一律以 FR24 dashboard 為準。

### 📅 2026-08 月度計劃（本期 666,000 credits，全押戰役）

**今日 dry-run 實測（0 credits）**：

| 範圍 | 待抓筆數 | credits |
|---|---|---|
| Batch 1（rank 1–39） | 3,273 | 130,920 |
| └ 其中亞洲 11 座 megahub | 2,743 | 109,720 |
| Batch 2（rank 40–125） | 16,719 | 668,760 |
| └ 其中字母序尾端亞太 27 座 | **7,716** | **308,640** |
| B1+B2 聯集（重疊僅 750 筆） | 19,242 | 769,680 |

**三段執行（順序不可調換，理由見下方字母序陷阱）**：

| 段 | 範圍 | 筆數 | credits | 時數 | 產出 |
|---|---|---|---|---|---|
| 1 | `--rank 1-39` 全清 | 3,273 | 130,920 | ~2.6h | **Batch 1 100% — 全球 39 大 megahub 全滿** |
| 2 | Batch 2 亞太尾端 27 座（`--airports`） | ~7,700 | ~308,600 | ~6.1h | 中國／東南亞／印度／澳洲成網 → 中途部署 |
| 3 | Batch 2 其餘（歐美中東），吃滿剩餘 | ~5,400 | ~226,400 | ~4.4h | Batch 2 約 8 成 |

合計 **~16,600 筆 / ~13 小時純抓取** → 進度 **28.6% → 約 53–54%（戰役過半）**。
展望：照 666K/月，9 月週期 → ~79%、10 月週期收官。

```bash
# 段 1：Batch 1 全清（額度蓋得住整批，字母序無所謂）
...
caffeinate -dims nohup npx tsx scripts/fetch-tracks.ts \
  --airports RPLC,RPVM,VCBI,VDTI,VGHS,VOBL,VOCI,VOHS,VOMM,VRMM,VVCI,VVCT,VVPQ,WADD,WMKP,YMML,YSSY,ZHHH,ZLXY,ZPPP,ZSAM,ZSFZ,ZSHC,ZSQD,ZUCK,ZYHB,ZYTL \
  --from-time 2026-02-17T16:00:00Z --to-time 2026-02-18T16:00:00Z \
  --max-credits 308640 > scripts/_seg2.log 2>&1 &

# 段 3：Batch 2 其餘 —— 開跑前重新 dry-run 取數，--max-credits = 剩餘額度 × 0.95
npx tsx scripts/campaign-status.ts   # 看剩餘額度，再算 cap
```
...
- **上輪實證（8/09）**：`--rank 1-39` 抓 10,785 筆，亞洲 12 座 megahub 只吃到 225 筆（2.1%），北美（K 開頭）吃掉大半。
- **本輪風險更大**：Batch 2 的待抓量有 **46%（7,716 / 16,719）壓在尾端那 27 座亞太機場**。若照 `--rank 40-125` 直接跑，本月吃不完的 ~2,600 筆缺口會**整批落在中國**。
- **對策**：段 2 用 `--airports` 把亞太 27 座前置，段 3 才吃歐美中東剩餘。**✅ 這招 8/09 已在 ZUTF 成都天府驗證**（802 筆單點清空，首座全滿的亞洲 megahub）。

> 💡 **通用教訓**：任何「額度只夠抓一部分」的批次，先確認處理順序會不會讓某地理區整批落在尾端。`--rank` 分批不保證地理均勻。
> 💡 **額度零碎時**：與其 N 座各咬一口，不如挑一座「待抓量 ≤ 額度」的機場單點清空 —— 一座 100% 的 megahub 比 N 座各半更有價值。挑選法：候選機場逐一 `--airports <單座> --dry-run`（0 credits）比對待抓量。
...
| 批次 | rank 帶 | 原始規劃 | **剩餘 @ 2026-08-09** | 剩餘 credits |
|------|---------|---------|---------|---------|
| **Batch 1（目前批次）** | 1–39 megahub | 15,158 | **2,590** | 104K |
| Batch 2 | 40–125 | 15,040 | 12,523 | 501K |
| Batch 3 | 126–395 | 15,003 | 13,224 | 529K |
| Batch 4 | 396–673 | 15,059 | 13,471 | 539K |
| Batch 5 | 674–1000 長尾 | 5,161 | 4,907 | 196K |

> ⚠️ 「剩餘」是 campaign-status 按**出發機場**歸屬算的；`--rank` 實際抓的是「起點**或**終點」命中，todo 會更大。Batch 1 的 dry-run 實測待抓 14,881（不是 10,286），差額是各地飛進 megahub 的班。**估額度一律以 `--dry-run` 為準，不要用這張表的數字。**
