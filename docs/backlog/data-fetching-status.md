...
> **狀態快照 @ 2026-08-09**：本輪吃掉用戶核准的 455K 額度（實花 432,280），戰役進度 **10.9% → 27.4%**（17,948/65,421）。額度剩 ~22,760，本期基本用盡；**下次開工先跟 FR24 dashboard 對帳，再更新 `campaign-top1000.json` 的 `manual_remaining` / `as_of` / `track_done_baseline`（三個都要，baseline 不同步會把上期花費重複扣一次）**。
...
**對策**：下輪**先跑亞洲專場**，用 `--airports` 繞開字母序，再跑 `--rank 1-39` 吃剩餘額度：

```bash
# 段 1：亞洲 12 座 megahub（3,545 筆 ≈ 141,800 credits，約 2.8 小時）
npx tsx scripts/fetch-tracks.ts \
  --airports RPLL,WMKK,VVTS,ZGGG,VVNB,VMMC,ZGSZ,VIDP,VABB,WIII,VVDN,ZUTF \
  --from-time 2026-02-17T16:00:00Z --to-time 2026-02-18T16:00:00Z \
  --max-credits 141800

# 段 2：Batch 1 剩餘（3,057 筆）＋ 之後推進 Batch 2，吃滿剩下額度
...
|------|---------|---------|---------|---------|
| **Batch 1（目前批次）** | 1–39 megahub | 15,158 | **3,057** | 122K |
| Batch 2 | 40–125 | 15,040 | 12,589 | 504K |
| Batch 3 | 126–395 | 15,003 | 13,310 | 532K |
| Batch 4 | 396–673 | 15,059 | 13,586 | 543K |
| Batch 5 | 674–1000 長尾 | 5,161 | 4,931 | 197K |
...
### 2026-08-09: 🌐 Batch 1 大推進 — 10,785 筆 / 432K credits，戰役 10.9% → 27.4%
