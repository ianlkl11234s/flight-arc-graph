...
### 🔴 下輪第一優先：亞洲 megahub 專場（字母序陷阱）

**問題**：`fetch-tracks` 的處理順序 = `readdirSync(scripts/flights)` 的 **ICAO 字母序**，不是 rank 序、也不是地理輪流。字母序尾端剛好是 **V（印度／越南）、W（印尼／馬來西亞）、Z（中國）** —— 額度用完時被砍的全是亞洲。

**實證**（2026-08-09 本輪）：亞洲 12 座 megahub 開跑前待抓 3,770 筆，跑完 10,785 筆後只剩 3,545 —— **本輪只抓到 225 筆（2.1%）**，而北美（K 開頭）吃掉大半額度。VIDP 德里、VABB 孟買、WIII 雅加達、ZUTF 成都幾乎原地不動。

**對策**：下輪**先跑亞洲專場**，用 `--airports` 繞開字母序，再跑 `--rank 1-39` 吃剩餘額度。**✅ ZUTF 成都已於 8/09 用這招單點清空**（802 筆，證明有效）；剩下 11 座：

```bash
# 段 1：亞洲 11 座 megahub（2,743 筆 ≈ 109,720 credits，約 2.2 小時）
npx tsx scripts/fetch-tracks.ts \
  --airports RPLL,WMKK,VVTS,ZGGG,VVNB,VMMC,ZGSZ,VIDP,VABB,WIII,VVDN \
  --from-time 2026-02-17T16:00:00Z --to-time 2026-02-18T16:00:00Z \
  --max-credits 109720   # 先 --dry-run 確認實際 todo

# 段 2：Batch 1 剩餘（2,590 筆）＋ 之後推進 Batch 2，吃滿剩下額度
npx tsx scripts/campaign-status.ts   # 照它印的建議指令跑
```

> 💡 **額度零碎時的用法**：與其 N 座各咬一口，不如挑一座「待抓量 ≤ 額度」的機場單點清空 —— 拿到一座 100% 的 megahub，視覺與里程碑價值都比 N 座各半高。8/09 的 875 筆額度就是這樣換成完整的成都天府。挑選法：對候選機場逐一 `--airports <單座> --dry-run`（0 credits）比對待抓量。

> 💡 這是**通用教訓**，不只這次：任何「額度只夠抓一部分」的批次，都要先確認處理順序會不會讓某個地理區整批落在尾端。單靠 `--rank` 分批不足以保證地理均勻。
