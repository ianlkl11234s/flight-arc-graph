...
> **狀態快照 @ 2026-08-21（本期額度用盡，剩 ~40）**：戰役 **34,748 / 65,421 = 53.1%（過半）**，還剩 30,673 條 ≈ 1.23M credits。8/20–21 共抓 **16,312 筆**、花 **~670,440 credits**、**零最終失敗**（16 筆 5xx／網路失敗全數 retry 救回）。
> **✅ 全鏈已上線**（S3 → Zeabur，2,299 座機場檔／82,091 不重複航班），線上站台即最新。
> 🔜 **下次開工（新週期 ~9/20 重置）**：先跟 FR24 dashboard 對帳 → 更新 `campaign-top1000.json` 三欄（`manual_remaining` / `as_of` / `track_done_baseline=82202`，三個一起改）→ 跑 `campaign-status.ts`。
> 💰 成本：時刻表 **3 credits/筆**、軌跡 **40 credits/筆**；估算一律以 FR24 dashboard 為準。

### 📅 2026-08 週期成果（8/20–21，計劃三段全部執行完畢）

| 段 | 範圍 | 處理 | 成功 | 空軌跡 | 失敗 | credits | 時數 |
|---|---|---|---|---|---|---|---|
| 1 | `--rank 1-39` 全清 | 3,273 | 3,270 | 2 | 1（救回）| 130,960 | 2.6h |
| 2 | Batch 2 亞太尾端 27 座 | 6,984 | 6,566 | 415 | 3（救回）| 279,480 | 5.6h |
| 3 | Batch 2 其餘（歐美中東）| 6,189 | 6,161 | 27 | 12（救回）| 248,040 | 6.4h |
| 4 | 餘額清尾：MMUN 坎昆 + OPKC 喀拉蚩 | 299 | 299 | 0 | 0 | 11,960 | 0.2h |
| — | **合計** | **16,745** | **16,312** | **444** | **0（最終）** | **~670,440** | **14.8h** |

- **🏆 Batch 1 100% 完成**：全球運量前 39 大 megahub 在 2/18 的所有起降軌跡到位（`--rank 1-39 --dry-run` 複驗**待處理 0 筆**）。
- **進度**：28.6% → **53.1%**；機場檔 1,845 → **2,303 座**；不重複航班 66,078 → **82,390**；Atlas 2,250 → **2,623 點**；track-done 66,189 → **82,501**。
- **🎯 段 4「餘額清尾」**：用戶回報 dashboard 尚餘 12,000（帳本估 7,720，差額以 dashboard 為準）→ 照「單點清空優於各咬一口」原則，逐座 `--dry-run` 篩出待抓量 ≤300 的候選（MMUN 219／LTFJ 439／MMMX 457／ZHHH 0），選 **MMUN 坎昆 219 筆（8,760）+ OPKC 喀拉蚩 80 筆（3,200）= 11,960**，兩座**都 100% 清空、零空零失敗**、複驗皆 0 筆，額度只剩 ~40。
- **Region 變化**：other 28,591 → 37,701｜US 22,314 → 26,055｜**CN 8,369 → 12,211**｜UK 3,899 → 4,223｜TW 12,215 → 12,230。
- **空軌跡 444 筆（2.7%）**：段 2 的中國段佔絕大多數（5.9%），與 ZUTF 8/09 實測 5.2% 一致 —— ADS-B 地面站覆蓋稀，**照樣計費、重抓無用、不要 retry**。
...
| 優先 | 範圍 | 待抓（現值）| credits |
|---|---|---|---|
| 1 | Batch 2 收尾 `--rank 40-125` | 2,647 | ~106K |
| 2 | Batch 3 `--rank 126-395` | 11,261 | ~450K |
| 3 | Batch 4 起頭 | 11,669 | ~467K |

666K 額度可清完 Batch 2 + Batch 3 大半 → 預估推進到 **~72%**。⚠️ 老規矩：**估額度一律以 `--dry-run` 為準**（表列是「出發機場」歸屬，實抓是「起點或終點」命中）；Batch 3 有 270 座機場，開跑前先確認字母序尾端（V/W/Y/Z）的待抓佔比，必要時比照本輪用 `--airports` 前置。

### 🔴 字母序陷阱（通用教訓，已驗證兩輪）

`fetch-tracks` 的處理順序 = `readdirSync(scripts/flights)` 的 **ICAO 字母序**，不是 rank、也不是地理輪流。尾端是 **R / V / W / Y / Z**（菲律賓、越南、印度、印尼、馬來西亞、澳洲、中國）。

- **反例（8/09）**：`--rank 1-39` 抓 10,785 筆，亞洲 12 座 megahub 只吃到 225 筆（2.1%），北美（K 開頭）吃掉大半。
- **正例（8/20–21）**：段 2 先用 `--airports` 跑亞太 27 座（Batch 2 待抓量的 46%），全部清空；額度耗盡時被砍的是歐美零散航班，無地理黑洞。
- **判斷法**：任何「額度只夠抓一部分」的批次，開跑前先對尾端機場做 `--airports <清單> --dry-run`，看佔比決定要不要前置。

> 💡 **額度零碎時**：與其 N 座各咬一口，不如挑一座「待抓量 ≤ 額度」的機場單點清空 —— 一座 100% 的 megahub 比 N 座各半更有價值（8/09 的 ZUTF 成都天府即此法）。

### ⏸️ 待抓：Top-1000 軌跡分批（P2）— Batch 1 已完成，續推 Batch 2–5
...
| 批次 | rank 帶 | 原始規劃 | **剩餘 @ 2026-08-21** | 剩餘 credits |
|------|---------|---------|---------|---------|
| **Batch 1** ✅ | 1–39 megahub | 15,158 | **0**（dry-run 複驗）| — |
| **Batch 2（目前批次）** | 40–125 | 15,040 | **2,647** | 106K |
| Batch 3 | 126–395 | 15,003 | 11,261 | 450K |
| Batch 4 | 396–673 | 15,059 | 11,669 | 467K |
| Batch 5 | 674–1000 長尾 | 5,161 | 4,427 | 177K |

> ⚠️ 「剩餘」是 campaign-status 按**出發機場**歸屬算的；`--rank` 實際抓的是「起點**或**終點」命中，兩者對不上是常態（8/09 Batch 1 表列 10,286、dry-run 14,881；8/21 Batch 1 表列仍印 727、dry-run 卻是 0），差額是各地飛進 megahub 的班。**估額度一律以 `--dry-run` 為準，不要用這張表的數字。**
...
### 2026-08-09（第二段）: ✅ ZUTF 成都天府 100% 清空 — 首座全滿的亞洲 megahub

- 用戶回報 dashboard 尚餘 35,000 credits（875 筆）→ 與其 12 座各咬一口，**挑 ZUTF（待抓 802 筆 = 32,080）單點清空**。結果：**802 筆全處理、760 有軌跡、42 空、零失敗**，dry-run 複驗待處理 **0 筆**。
...
- **抓取**：`--rank 1-39 --max-credits 432250`，8.7 小時（2.84 秒/筆，`caffeinate -dims nohup` detach 全程無中斷）。**10,784 成功、21 空軌跡、1 筆 API 502**（已用 retry-failed-tracks 救回 554 點，**最終失敗率 0**）。實花 432,280 credits（432,240 + retry 40）。
- **成果**：track-done 54,644 → **65,429**；manifest 不重複航班 54,533 → **65,318**；機場 JSONL **1,594 → 1,829 座**（+235 座是軌跡雙寫順帶織進來的非目標機場，0 額外成本）；Atlas 點位 2,050 → 2,235。
- **抓取正確性已驗證**（抽樣 130 筆 100% 命中）：時間窗、機場命中、零重抓、高度單位（公尺）、時間戳（unix 秒）全部正常，7 月那兩個事故沒有復發。
- **⚠️ 高度 spike 是資料源雜訊不是 bug**：split-tracks 警告 3 個點 alt > 16,000 m（AAL2923 的 A319 一個點 19,690 m、GOHAC 的 C182 一個點 17,648 m）。單位錯誤會讓**整條**軌跡偏移 3.28×，這裡是數千萬點中的 3 個孤立 spike = ADS-B 訊號雜訊，不需處理。
- **🔴 教訓：字母序讓亞洲整批落榜** → 見上方「下輪第一優先」段。亞洲 12 座 megahub 本輪只抓到 225 筆（2.1%）。
- **額度錨點機制的坑**：更新 `manual_remaining` 時**必須同步更新 `track_done_baseline`**，否則 campaign-status 的「track-done 差值」自癒邏輯會把上一期已花的 credits 重複扣在新錨點上（本次若沒改，455K 會被誤算成 330,880）。
